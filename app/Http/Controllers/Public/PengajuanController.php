...
public function store(StorePengajuanRequest $request)
{
    ...
    // The unique code is generated from existing rows. Retrying only this
    // known constraint protects concurrent submissions without hiding other
    // database errors.
    for ($attempt = 0; $attempt < 3; $attempt++) {
        try {
            ...
            $isApplicationCodeConflict = str_contains(
                strtolower($exception->getMessage()),
                'application_code'
            );

            if (! $isApplicationCodeConflict || $attempt === 2) {
                throw $exception;
            }
        }
    }
