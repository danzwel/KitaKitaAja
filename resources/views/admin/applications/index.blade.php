<x-admin.layouts.app title="Dashboard">
    ...
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-6">
        @php
            $cards = [
                ...
                ['label' => 'Menunggu', 'value' => $stats['pending'], 'icon' => 'ti-clock', 'accent' => 'text-[#5B6660] bg-[#EFF1EC]'],
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-5">
        {{-- Chart --}}
        <x-admin.card title="Grafik Pengajuan" subtitle="Jumlah pengajuan per bulan, tahun {{ now()->year }}" class="xl:col-span-3">
            ...
            <div class="divide-y divide-[#E3E5DE]">
                @forelse ($latestApplications as $application)
                    <a href="{{ route('admin.applications.show', $application) }}" class="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0 hover:opacity-80">
                        <div class="min-w-0">
                            <p class="truncate text-sm font-medium text-[#1E2A24]">{{ $application->name }}</p>
                            <p class="truncate text-xs text-[#64705F]">{{ $application->department->name ?? '-' }} · {{ $application->application_date?->translatedFormat('d M Y') }}</p>
                        </div>
