    ...
    <x-admin.card title="Tambah Bidang Magang" class="max-w-2xl">
        <form method="POST" action="{{ route('admin.departments.store') }}" class="space-y-4">
            ...
            <label class="flex items-center gap-2 text-sm text-[#4B564B]">
                <input type="checkbox" name="requires_portfolio" value="1" @checked(old('requires_portfolio')) class="rounded border-[#E3E5DE] text-[#0C2340] focus:ring-[#0C2340]">
                Memerlukan portofolio
            </label>

            <label class="flex items-center gap-2 text-sm text-[#4B564B]">
                <input type="checkbox" name="is_active" value="1" checked class="rounded border-[#E3E5DE] text-[#0C2340] focus:ring-[#0C2340]">
                Aktifkan bidang ini
            </label>
            ...
            <div class="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" class="rounded-lg bg-[#0C2340] px-4 py-2.5 text-sm font-medium text-white hover:bg-[#081A30]">Simpan</button>
            </div>
        </form>
    </x-admin.card>
</x-admin.layouts.app>
