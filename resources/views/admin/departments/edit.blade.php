<x-admin.layouts.app title="Edit Bidang">
    ...
    <x-admin.card title="Edit Bidang Magang" class="max-w-2xl">
        <form method="POST" action="{{ route('admin.departments.update', $department) }}" class="space-y-4">
            ...
            <div>
                <label class="mb-1.5 block text-sm font-medium text-[#1E2A24]">Deskripsi</label>
                <textarea name="description" rows="4"
                          class="w-full rounded-lg border border-[#E3E5DE] px-3 py-2.5 text-sm focus:border-[#0F6E56] focus:outline-none focus:ring-1 focus:ring-[#0F6E56]">{{ old('description', $department->description) }}</textarea>
            </div>

            <label class="flex items-center gap-2 text-sm text-[#4B564B]">
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $department->is_active)) class="rounded border-[#E3E5DE] text-[#0F6E56] focus:ring-[#0F6E56]">
                Aktifkan bidang ini
            </label>
